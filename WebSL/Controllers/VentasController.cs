    ...
    public class VentasController : Controller
    {
        ...
        public void GenerarExcel()
        {
            ...
            foreach (var item in data)
            {
                ...
                AddValue(row, cellnum++, item.Cod_Venta.ToString(), styleBody, sheet);
                AddValue(row, cellnum++, item.Producto.Marca_Prod.ToString(), styleBody, sheet);
                AddValue(row, cellnum++, item.Precio_Prod.ToString("F2"), styleBody, sheet); 
                AddValue(row, cellnum++, item.Precio_Venta.ToString("F2"), styleBody, sheet);
                AddValue(row, cellnum++, item.Descuento_Venta.ToString("F2"), styleBody, sheet);
                AddValue(row, cellnum++, item.Precio_Final.ToString("F2"), styleBody, sheet); 
